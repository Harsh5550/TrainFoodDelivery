...
public class DeliveryMapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    private void listener(){
        binding.confirmLocation.setOnClickListener(v->{
            if (distanceValid()){
                Intent intent=new Intent(getApplicationContext(), DeliverySignUp.class);
    ...
    public void onMapReady(@NonNull GoogleMap googleMap) {
        mMap = googleMap;
        Toast.makeText(this, "Set your Permanent Address", Toast.LENGTH_SHORT).show();
        try {
            Intent intent1 = getIntent();
            Geocoder geocoder = new Geocoder(DeliveryMapsActivity.this, Locale.getDefault());
