...
import com.harsh.trainfooddelivery.databinding.DeliveryChangePasswordBinding;
...
public class DeliveryChangePassword extends AppCompatActivity {
    private DeliveryChangePasswordBinding binding;
    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding=DeliveryChangePasswordBinding.inflate(getLayoutInflater());
    ...
    private void changePassword(){
        ...
        try {
            database.collection(Constants.KEY_COLLECTION_DELIVERY)
                    .whereEqualTo(Constants.KEY_DELIVERY_PHONE_NUMBER, AESCrypt.encrypt("frmeihafokfso", phone))
                    .get()
                        ...
                        if (userId[0]!=null) {
                            try {
                                database.collection(intent.getStringExtra("city"))
                                        .document(userId[0])
                                        .update(Constants.KEY_DELIVERY_PASSWORD, AESCrypt.encrypt("frmeihafokfso", binding.inputPassword.getText().toString()))
                                        .addOnSuccessListener(v -> {
                                                    loading(false);
                                                    Intent i = new Intent(getApplicationContext(), DeliverySignIn.class);
